...
namespace sharpEnumAv
{
    class Program
    {
        ...
        {
            string txt = @"""360tray.exe"": ""360安全卫士-实时保护"",
...
""wdswfsafe.exe"": ""360杀毒-网盾""";
            ...
            Dictionary<string, string> avdic = new Dictionary<string, string>();
            Dictionary<string, string> result = new Dictionary<string, string>();
            foreach (var line in txt.Split(','))
            {
                string key = line.Split(':')[0].Trim();
                string value = line.Split(':')[1].Trim();
                avdic.Add(key.Substring(1, key.Length - 2), value.Substring(1, value.Length - 2));
            }
            foreach (System.Diagnostics.Process p in System.Diagnostics.Process.GetProcesses())
            {
                string proname = p.ProcessName + ".exe";
                if (avdic.ContainsKey(proname))
                {
                    if (!result.ContainsKey(avdic[proname]))
                    {
                        result.Add(avdic[proname], proname);
                    }
                    else
                    {
                        result[avdic[proname]] += $",{proname}";
                    }
                }
            }
            foreach (var av in result)
            {
                ...
                Console.WriteLine($"{av.Key}--{av.Value}");
            }
        }
    }
